...
!--- radiation control structure
 ...
 type radiation_input_control_type
     ...
     logical :: do_uni_zfull             ! miz added from AM4
...
!---------------------------------------------------------------------
 ...
 subroutine alloc_radiation_type (Radiation, Atm_block, p_hydro, do_uni_zfull)
  type (radiation_type), intent(inout) :: Radiation
  type (block_control_type), intent(in) :: Atm_block
  logical,               intent(in)    :: p_hydro
  logical,               intent(in) :: do_uni_zfull !< Added, but not Used ter 8/24/2016
!--- local varialbes
