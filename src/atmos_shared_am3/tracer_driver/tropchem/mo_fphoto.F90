...
!----------------------------------------------------------------------
      use mpp_mod,              only : mpp_error, mpp_pe, fatal
...
!      end if
      ...
      v1(:,:) = so4(:,:)/denso4
      v2(:,:) = (bc1(:,:)+bc2(:,:))/denbc
!> TER: New way to calculate v1 here.  Avoids dividing by 0
do i=1,size(v1,dim=1) ; do j=1,size(v1,dim=2)
       if (v1(i,j) .ne. 0.000) v1(i,j) = v1(i,j)/(v1(i,j)+v2(i,j))*100.
!      v1(:,:) = v1(:,:)/(v1(:,:)+v2(:,:))*100.
enddo ; enddo
