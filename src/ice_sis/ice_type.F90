...
public  :: iceClocka,iceClockb,iceClockc
  ...
  real    :: max_ice_alb    = 0.80       ! maximum ice albedo
  real    :: min_ice_alb    = 0.55       ! minimum ice albedo
  ...
  type ice_data_type
     ...
     type(domain2D)                     :: slow_Domain_NH ! A copy of Ice%Domain.
     ...
     integer, pointer, dimension(:)     :: slow_pelist         =>NULL() ! Used for flux-exchange.
     integer, pointer, dimension(:)     :: fast_pelist         =>NULL() ! Used for flux-exchange.
     ...
     real,    pointer, dimension(:,:)   :: SST_C               =>NULL()
  ...
  subroutine ice_model_init (Ice, Time_Init, Time, Time_step_fast, Time_step_slow, &
    ...
    if (present(Verona_coupler)) then ; if (.not.Verona_coupler) then
      if (Ice%fast_ice_pe .NEQV. Ice%slow_ice_PE) call error_mesg('ice_model_init', &
        'SIS1 does not support separate fast and slow ice PEs.', FATAL)
    endif ; endif
    ...
    call ice_thm_param(alb_sno, alb_ice, pen_ice, opt_dep_ice, slab_ice, &
                       t_range_melt, max_ice_alb, min_ice_alb, &
                       cm2_bugs, ks, h_lo_lim)
...
!
  ...
  subroutine ice_model_end (Ice)
    ...
    deallocate(Ice % vmask, Ice % SST_C)
...
100 FORMAT("   CHECKSUM::",A32," = ",Z20)
    ...
    write(outunit,100) 'ice_data_type%SST_C              ',mpp_chksum(data_type%SST_C(isc:iec,jsc:jec)               )
